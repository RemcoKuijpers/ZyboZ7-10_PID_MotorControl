...
ENTITY quadrature_decoder IS
	...
	GENERIC(
		...
		positions					:	INTEGER := 8000; 		--size of the position counter (i.e. number of positions counted)
		debounce_time				:	INTEGER := 250; 	--number of clock cycles required to register a new position = debounce_time + 2
		set_origin_debounce_time	:	INTEGER := 500000);	--number of clock cycles required to register a new set_origin_n value = set_origin_debounce_time + 2
...
ARCHITECTURE logic OF quadrature_decoder IS
	...
	SIGNAL  position	        :	INTEGER RANGE -4000 TO 4000 := 0;	--current position relative to index or initial value	
			...
			ELSIF(debounce_cnt = debounce_time													--debounce time for a and b is met
				...
				IF((b_prev XOR a_new(1)) = '1') THEN												--clockwise direction
					...
					ELSE																							--at position limit
						position <= position + 1;--position <= 0;																				--roll over position counter to zero
					END IF;
				ELSE																							--counter-clockwise direction
					IF(position > 0) THEN																	--not at position limit
						position <= position - 1;																--decrement position counter
					ELSE																							--at position limit
						position <= position - 1;--position <= positions-1;																--roll over position counter maximum
					END IF;
				END IF;
			END IF;

		  pos <= std_logic_vector(to_signed(position, 16));
		END IF;
	END PROCESS;
END logic;
